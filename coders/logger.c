#include <sys/time.h>
#include <stdio.h>
#include <pthread.h>
#include "logger.h"

long	get_time(void)
{
	struct timeval	tv;

	gettimeofday(&tv, NULL);
	return (tv.tv_sec * 1000 + tv.tv_usec / 1000);
}

static const char	*get_message(int message_type)
{
	if (message_type == TOOK_DONGLE)
		return ("has taken a dongle");
	if (message_type == COMPILING)
		return ("is compiling");
	if (message_type == DEBUGGING)
		return ("is debugging");
	if (message_type == REFACTORING)
		return ("is refactoring");
	return ("burned out");
}

void	log_action(t_simulation *sim, int coder_id, int message_type)
{
	long	timestamp;

	pthread_mutex_lock(&sim->print_mutex);
	if (sim->is_running == 0)
	{
		pthread_mutex_unlock(&sim->print_mutex);
		return ;
	}
	timestamp = get_time() - sim->start_time;
	printf("%ld %d %s\n", timestamp, coder_id, get_message(message_type));
	if (message_type == BURNED_OUT)
		sim->is_running = 0;
	pthread_mutex_unlock(&sim->print_mutex);
}
